...
def main():
    ...
    submissions = process_all_documents(args.submissions_folder)
    ...
    answer_key_md = process_single_document(args.answer_key)
    ...
    if args.rubric:
        if not os.path.isfile(args.rubric):
            ...
            rubric_md = process_single_document(args.rubric)
            if not rubric_md:
